...
import requests, openpyxl, time, shutil, sys, re, json, os
...
def mode_comprova_publicar():
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        data  = v(row[I_DATA])
        estat = v(row[I_ESTAT])
        origen = v(row[I_ORIGEN])
        # Novetat validada = té data + estat buit + origen CIMA
        if data and not estat and "cima.aemps.es" in origen:
            pendents += 1
...
def mode_regenera():
    """
    FUSIÓ: conserva els fàrmacs originals del HTML intactes +
    afegeix les novetats validades de l'Excel al final del catalog[].

    Una fila és "novetat validada" si:
      - Col N (estat) = buida  → validada per la Sílvia
      - Col M (origen) conté URL cima.aemps.es → prové del --detecta
      - Col L (data) té data → posada pel --detecta
    """
    print("🔄 Mode: regenerar HTML")
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        if estat:                              continue  # pendent o no incorporar
        if "cima.aemps.es" not in origen:     continue  # fàrmac original → el HTML ja el té
        if not data_l:                         continue  # sense data → original

        cat   = v(row[I_CLASSE])
        ia    = v(row[I_IA])
        if not cat or not ia:                  continue
        ...
        starred = bool(phf and phf.strip() not in ("","None"))
        matma_b = bool(matma and matma.strip() not in ("","None",""))
        ...
        if dosi:
            ...
            for line in lines:
                line = line.strip()
                if line.startswith("MPOC:"):
                    farmac["mpoc"] = line.replace("MPOC:","").strip()
                elif line.startswith("D.màx:"):
                    farmac["dmx"] = line.replace("D.màx:","").strip()
                elif line.startswith("Asma baixa:"):
                    farmac["asma_baixa"] = line.replace("Asma baixa:","").strip()
                elif line.startswith("Asma mitjana:"):
                    farmac["asma_mitjana"] = line.replace("Asma mitjana:","").strip()
                elif line.startswith("Asma alta:"):
                    farmac["asma_alta"] = line.replace("Asma alta:","").strip()
                elif not any(farmac.get(k) for k in ["mpoc","dose"]):
                    farmac["dose"] = line

        novetats.append(farmac)
        print(f"  ✅ {brand[:50]} | {cat} | {tipus}")

    wb.close()

    if not novetats:
        print("  ℹ️  Cap novetat validada per publicar.")
        with open("publicar_count.txt","w") as f: f.write("0")
        return

    print(f"\n  Total novetats a publicar: {len(novetats)}")

    # Llegir HTML actual
    with open(HTML_PATH, "r", encoding="utf-8") as f:
        html = f.read()

    # Generar JS per a cada novetat
    def farmac_a_js(d):
        parts = []
    ...
    # Fusionar: catalog actual + novetats al final
    pattern = re.compile(r"(var catalog=\[)(.*?)(\];)", re.DOTALL)
    match = pattern.search(html)
    if not match:
        print("  ❌ ERROR: No s'ha trobat 'var catalog=[...];' al HTML.")
        return

    catalog_actual = match.group(2).strip()
    n_actuals = catalog_actual.count("{cat:")
    novetats_js = ",\n" + ",\n".join([farmac_a_js(f) for f in novetats])
    ...
    print(f"  ✅ HTML actualitzat: {n_actuals} originals + {len(novetats)} novetats = {n_actuals + len(novetats)} fàrmacs")
